...
if [ "$1" = "prodonly" ]; then
    ...
    envsubst '$$PORT $$DEV_PORT $$DOMAIN_NAME $$DEV_DOMAIN_NAME $$FRONTEND_DEV_ADDRESS $$BACKEND_ADDRESS $$BACKEND_DEV_ADDRESS' < /etc/nginx/nginx.conf.template > /etc/nginx/nginx.conf
fi
